...
describe('NetworkController provider tests', () => {
  ...
  describe('if NetworkController is configured with an Infura network', () => {
      ...
      describe('if the given getPendingTransactionByHash function returns nothing', () => {
        it('passes the request through to Infura', async () => {
          ...
          mockSuccessfulRpcCallToInfura({
            ...
            rpcResponse: { result: 'result from Infura' },
          });
    ...
    // Outstanding questions / things to investigate:
    //
    // * What happens if an RPC method is called twice with two different
    //   arguments? Is the second still cached?
    // * What happens if the result of an RPC call is null or undefined? Is it
    //   still cached?
    //
    // Things to do:
    //
    // * Make sure that no RPC methods are duplicated, reorg by that
